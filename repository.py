...
class TransactionRepository:
    ...
    def cargar(self) -> list[Transaccion]:
        ...
        try:
            ...
            raise RuntimeError(f"Error al leer el historial: {e}") from e

    def guardar(self, transacciones: list[Transaccion]) -> None:
        try:
            with open(self._ruta, "w", encoding="utf-8") as f:
                json.dump([t.to_dict() for t in transacciones], f, indent=2, ensure_ascii=False)
        except OSError as e:
            raise RuntimeError(f"Error al guardar el historial: {e}") from e
