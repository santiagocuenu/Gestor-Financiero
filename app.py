...
class AppFinanzas:
    """Capa de presentación. Delega toda la lógica a FinanceService."""
    ...
    def _construir_header(self) -> None:
        ...
        tk.Label(frame, text="GESTOR FINANCIERO", font=("Segoe UI", 24, "bold"),
                 bg=TEMA["principal"], fg=TEMA["texto"]).pack(pady=15)
        tk.Label(frame, text="Tu historial financiero, guardado y organizado",
                 font=("Verdana", 10), bg=TEMA["principal"], fg=TEMA["secundario"]).pack()

    def _construir_formulario(self) -> None:
        frame = tk.Frame(self._root, bg=TEMA["principal"])
        frame.pack(fill="x", padx=20)

        tk.Label(frame, text="1. Seleccione el tipo:", font=("Segoe UI", 11, "bold"),
                 bg=TEMA["principal"], fg=TEMA["texto"]).pack(anchor="w", pady=(10, 4))
        self._combo_tipo = ttk.Combobox(frame, values=["Ingreso", "Gasto"],
                                         state="readonly", width=30, font=("Segoe UI", 10))
        self._combo_tipo.pack(anchor="w", fill="x", pady=(0, 12))

        tk.Label(frame, text="2. Ingrese el monto:", font=("Segoe UI", 11, "bold"),
                 bg=TEMA["principal"], fg=TEMA["texto"]).pack(anchor="w", pady=(0, 4))
        self._entry_monto = tk.Entry(frame, font=("Calibri", 11), width=30,
                                      bg=TEMA["fondo_entry"], fg=TEMA["texto"],
                                      insertbackground=TEMA["secundario"])
        self._entry_monto.pack(anchor="w", fill="x", pady=(0, 12))

        tk.Label(frame, text="3. Ingrese la categoría:", font=("Segoe UI", 11, "bold"),
                 bg=TEMA["principal"], fg=TEMA["texto"]).pack(anchor="w", pady=(0, 4))
        self._entry_categoria = tk.Entry(frame, font=("Calibri", 11), width=30,
                                          bg=TEMA["fondo_entry"], fg=TEMA["texto"],
                                          insertbackground=TEMA["secundario"])
    ...
    def _construir_botones(self) -> None:
        ...
        for texto, bg, fg, comando in botones:
            tk.Button(frame, text=texto, bg=bg, fg=fg, font=("Segoe UI", 10, "bold"),
                      command=comando, relief="flat", padx=10, pady=8).pack(fill="x", pady=3)

    def _construir_pantalla(self) -> None:
        frame = tk.Frame(self._root, bg=TEMA["principal"])
        frame.pack(fill="both", expand=True, padx=20, pady=(10, 20))
        tk.Label(frame, text="Resultados:", font=("Segoe UI", 11, "bold"),
                 bg=TEMA["principal"], fg=TEMA["texto"]).pack(anchor="w", pady=(0, 5))
        self._pantalla = tk.Text(frame, height=10, width=50, state="disabled",
